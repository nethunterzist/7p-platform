-- 7P Education Platform - Latest Schema (DevOps Ready)
-- Complete schema with extensions, tables, indexes, RLS policies, and triggers
...
-- Public courses viewable by anyone
...
COMMENT ON TABLE public.users IS 'User profiles extending Supabase auth.users with FK constraint';
COMMENT ON TABLE public.courses IS 'Course catalog with pricing and instructor relationship';
COMMENT ON TABLE public.course_modules IS 'Course modules for organizing lessons hierarchically';
COMMENT ON TABLE public.lessons IS 'Individual lessons within course modules';
COMMENT ON TABLE public.course_enrollments IS 'Student course enrollments with progress tracking';
COMMENT ON TABLE public.lesson_progress IS 'Individual lesson completion and watch time tracking';
